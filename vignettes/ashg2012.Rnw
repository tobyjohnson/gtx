...
\date{presented at the 
  American Society of Human Genetics Annual Meeting,
  San Francisco, November 6--10, 2012 }
\maketitle
\thispagestyle{empty}
...
\begin{equation}
  ...
  \simeq \frac{\sum_{j=1}^m{w_j\hat\beta_js_j^{-2}\,n\,(\vec{z}'\vec{z})^{-1}}}
  {\sum_{j=1}^m{w_j^2s_j^{-2}\,n\,(\vec{z}'\vec{z})^{-1}}}\;=\;\tilde\alpha
...
\begin{center}
  ...
  \begin{minipage}{0.9\textwidth}
    \textsf{Figure~\arabic{figure}.~Stepwise removal of SNPs from the risk score, minimising
      ...
      removed SNPs at the \textit{GCKR}, \textit{TRIB1} and
      \textit{APOA5-A1} loci for triglycerides.  Estimated effects on
...
After removing SNPs that likely violate assumptions required for a
Mendelian randomisation argument, multi-SNP risk score analyses
suggest no causal effects of either HDL or triglycerides on CAD risk, with
narrow 95\% CIs 
(\Sexpr{oppc(hdl.grs2, +10, 0.025)}--\Sexpr{oppc(hdl.grs2, +10, 0.975)} 
odds for CAD per 10\% HDL increase, 
$P=\Sexpr{round(hdl.grs2$pval, 2)}$;
\Sexpr{oppc(tg.grs2, -10, 0.975)}--\Sexpr{oppc(tg.grs2, -10, 0.025)} 
odds for CAD per 10\% triglyceride decrease, 
$P=\Sexpr{round(tg.grs2$pval, 2)}$).\vspace{11pt}

By constructing a genetic risk score for HDL, excluding
``pleiotropic'' SNPs significantly associated (at $P\le0.01$) with
either LDL or triglycerides,
\cite{voight12:_plasm_hdl_choles_risk_myocar_infar} used the method
described here to support a related conclusion, that HDL has no causal
effect on myocardial infarction risk.  An advantage of the the
goodness of fit approach described here is that SNPs with
``pleiotropic'' effects on disease risk can be excluded from the risk
score, even when suitable markers for the ``pleiotropic'' effects are
not available.
