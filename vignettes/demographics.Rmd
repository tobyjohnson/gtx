...
The `demographics` functions in the GSKStatGen package aim to make
...
Different styles of numerical summaries can be specified in a straightforward and intuitive
...
dataset:
```{r prepare}
suppressPackageStartupMessages(library(GSKStatGen))
...
subjects in study AO000001 only, and a subset of columns for the
...
specified style will be used.  Hence, in the following, the "Mean
(IQR)" style will be used for all numeric variables except `bmi`, for
which the "Median (IQR)" style will be used.
...
`options(GSKStatGen.demographics.omit0groups = FALSE)` is used).
...
argument, logicals of hte wrong length, etc.), then a warning message
...
(GSKStatGen.demographics.omit0rows, default TRUE), and whether groups with no subjects are omitted 
(GSKStatGen.demographics.omit0groups, default TRUE),

In the future it is planned to provide automatic collapsing of levels
for factors with many levels, and automagically printing descriptive
variable names.
