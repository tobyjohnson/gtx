...
checkPhewasInput <- function(chrom, pos, ref, alt, rs, analysis,
                              analysis_not, description_contains,
                              phenotype_contains, has_tag, ncase_ge,
                              ncohort_ge, analysis_fields, tag_is, plot_ymax,
                              tags_to_right, dbc, connectionType, phewasObj = NULL){
...
getPhewasDataArgs <- function(phewasArgs){
  ...
  phewasDataArgsNames <- phewasDataArgsNames[phewasDataArgsNames != '']
  phewasDataArgs <- phewasArgs[names(phewasArgs) %in% phewasDataArgsNames]
...
phewasGetV1 <- function(connectionType = 'SQL', chrom, pos, ref, alt, rs,
                          dbc, uniq = FALSE, zrok = TRUE){
  #get query settings
  callArgs <- switch(connectionType,
                     'SQL' = getSQLArgsPhewas00(chrom = chrom, pos = pos,
                                                ref = ref, alt = alt, rs = rs,
                                                dbc = dbc,
                                        uniq = uniq, zrok = zrok),
                     stop('Unkown connectionType'))
  
  v1Data <- getDataFromDB(connectionType = connectionType,
                          connectionArguments = allArgs)
...
phewas.data <- function(chrom, pos, ref, alt, rs,
                        ...
                        analysis_fields = c('description', 'label', 'unit',
                                            'ncase', 'ncontrol', 'ncohort'),
                        tag_is,
                        with_tags = FALSE, # not clear why missing with_tags
  ...
  ## Look up variant
  v1 <- phewasGetV1(connectionType = 'SQL', chrom, pos, ref, alt, rs,
                      dbc, uniq = FALSE, zrok = TRUE)
...
getAnalysisMetadata(phewasObj){
  ...
  gtxanalysesArgsNames <- gtxanalysesArgsNames[gtxanalysesArgsNames != '']
  gtxanalysesArgs <- phewasDataArgs[names(phewasDataArgs) %in% gtxanalysesArgsNames]
  gtxanalysesArgs$has_access_only <- TRUE
  
  a1 <- do.Call(gtxanalyses, gtxanalysesArgs) # will work fine if all
...
getRes <- function(loop_clean_db, v1, uniq = FALSE, zrok = TRUE){
  ...
  res <- do.call(rbind, lapply(loop_clean_db, function(results_db) {
    ...
    #get query settings
    callArgs <- switch(connectionType,
                       'SQL' = getSQLArgsPhewas01(loop_clean_db, v1, uniq = FALSE, zrok = TRUE),
                       stop('Unkown connectionType'))
...
phewasDataProcessing <- function(phewasObj){
  ...
  a1 <- getAnalysisMetadata(phewasObj)
  ...
  if (is.null(v1)) {
    ...
    res <- switch(connectionType,
                  'SQL' = getRes(loop_clean_db, v1, uniq = FALSE, zrok = TRUE),
                  stop('Unkown connectionType'))
