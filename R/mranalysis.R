    ...
    if (rsid == FALSE) {
      if (is.null(analyses)) {
        ...
        if (is.data.frame(analyses)) {
          ...
          res_exp <- filter_entities(res_exp, analyses)
        }
        else{
          stop(
            "Error: 'analyses' [data.frame] should contain at least one column with analysis IDs if searching for specific studies."
          )
        }
      }
    }
      ...
      if (is.null(analyses)) {
        ...
        if (is.data.frame(analyses)) {
          ...
          stop(
            "Error: 'analyses' [data.frame] should contain at least one column with analysis IDs where fuzzy=FALSE."
          )
        }
      }
    }
...
#' Extracts top instruments based on the chosen p-value (the instruments are not pruned)
#' 
#' This function allows the users to extract the instruments
#' for molecular traits (e.g. transcripts or proteins) from the 
#' GWA studies database, which then can be used to run Mendelian 
      ...
      if (is.null(analyses_exp)) {
        if (!is.null(snp_list) & is.data.frame(snp_list)) {
          ...
          if (ncol(snp_list) > 1) {
            ...
            #Choose only the right <analysis,entity> pairs
            res_out <- filter_entities(res_out, analyses_out)
          ...
          stop(
            "Error: snp_list [data.frame] cannot be empty."
          )
        }
      } else{
        if (!is.null(snp_list))
          message("Warning! These instruments will not be considered; analyses_exp should be NULL.")
        ...
        if (is.data.frame(analyses_exp)) {
          ...
          stop(
            "Error: 'analyses_exp' [data.frame] should contain at least one column with 'analysis'."
          )
        }
      }
    }
      ...
      if (is.null(analyses_exp)) {
        if (!is.null(snp_list) & is.data.frame(snp_list)) {
          ...
          if (ncol(snp_list) > 1 & search == "pos:chrom") {
            ...
            n <-
              which(tolower(colnames(snp_list)) %like any% c("%rs%", "%snp%"))
            ...
            stop(
              "Error: snp_list should have either two columns in this order: position (1) and chromosome (2) or just one column with rsIDs (1)."
            )
          }
          ...
          stop(
            "Error: snp_list [data.frame] cannot be empty and should have at least two columns in this order: position (1) and chromosome (2)."
          )
        }
      } else{
        if (!is.null(snp_list))
...
format_for_mr <- function(data, type = "exposure", rsid = FALSE) {
  ...
  if (rsid == FALSE) {
    if (type == "exposure") {
      data <-
        data %>% dplyr::mutate(
          SNP = paste0(pos_index, ":", chrom, "_", ref_index, "_", alt_index),
          id = paste0(analysis, ":", entity)
        )
      ...
      data <-
        TwoSampleMR::format_data(
          ...
          pval_col = "pval_index",
          eaf_col = "freq_index",
          effect_allele_col = "alt_index",
          other_allele_col = "ref_index",
          samplesize_col = "ncohort"
        )
      ...
      data <-
        data %>% dplyr::mutate(
          SNP = paste0(pos, ":", chrom, "_", ref, "_", alt),
          id = paste0(analysis, ":", entity)
        )
...
run_mr_gsk <- function(exp,
    ...
    if (rsid == TRUE) {
      if (clump == TRUE)
        ...
        message("Warning! An automated LD-pruning of the exposure instruments is disabled.")
      
    } else{
      message(
        "Warning! An automated LD-pruning of the exposure instruments is disabled for the 'pos:chrom_ref_alt' identifiers."
      )
    }
...
visual_for_mr <- function(mrres,
    ...
    if (scatter == TRUE) {
      ...
      p1 <-
        p1[[1]] + labs(
          x = stringr::str_wrap(paste0("SNP effect on ", exp_label), width = 40),
          y = stringr::str_wrap(paste0("SNP effect on ", out_label), width = 35)
        )
    } else {
      p1 <- NULL
    ...
    if (forest == TRUE & !is.null(mrres[[4]])) {
      ...
      p2 <-
        p2[[1]] + labs(x = stringr::str_wrap(
          paste0("MR effect size for ", exp_label, " on ", out_label),
          width = 70
        ))
    } else {
      p2 <- NULL
...
#'@keywords internal
#'
format_plot_labels <- function(mrres,
                               exp_flag = "internal",
                               out_flag = "internal",
                               dbc = getOption("gtx.dbConnection", NULL)) {
    ...
    if (exp_flag == "internal") {
      ...
      exp_label <-
        gtxanalysis_label(analysis = exp_id,
                          nlabel = FALSE,
                          dbc = dbc)
    } else {
    ...
    if (out_flag == "internal") {
      ...
      out_label <-
        gtxanalysis_label(analysis = out_id,
                          nlabel = FALSE,
                          dbc = dbc)
    } else {
...
#' Choose the requested entities only
#' 
#' Removes the entities which were not in the list of requested studies.
#' 
#' @author Valeriia Haberland \email{valeriia.haberland@@bristol.ac.uk} 
#' @param res (required): an output of the database query
#' @param analyses (required): a dataframe of requested studies
#' @return a filtered query output with only entities indicated in 'analyses'
#' 
#' @keywords internal
#'
filter_entities <- function(res=NULL, analyses){
  
  #Choose only the right <analysis,entity> pairs
  if ((!is.null(res)) & (dim(res)[1] > 0)) {
    ids <-
      unique(analyses[, which(names(analyses) %in% c("analysis", "entity_type", "entity"))])
    if (is.data.frame(ids)) {
      colnames(ids)[2] <- "entity"
      res <- dplyr::left_join(res, ids)
      
    } else{
      message(
        "Warning! Entity cannot be identified; 'analyses' or 'analyses_out' dataframe is probably missing 'entity' column."
      )
    }
  } else{
    message("Warning! No result has been returned by the query.")
...
#' Validate an input for instrument extraction of molecular traits
#'
#' @author Valeriia Haberland \email{valeriia.haberland@@bristol.ac.uk}
#' @param analysis (required): a keyword/keyphrase to search for the study of interest
#' @param entities (default): other parameters 
#' 
#' @keywords internal
#' 
