...
piccolo <- function(chrom,pos,rs,pval,ancestry,indication,dbc=getOption("gtx.dbConnection", NULL)){
  ...
  for(j in 1:length(ens2List)){
    # TODO fix database
    tmp[[j]]<-sqlWrapper(dbc,paste0("SELECT * FROM gene_gwas_staging.pics_qtls_dev WHERE entity IN ('",paste(ens2List[[j]],collapse="', '"),"') "),uniq = F, zrok = FALSE)
  }
...
pics_calc <- function(index.data,dbc=getOption("gtx.dbConnection", NULL)){
  ...
  for(i in unique(index.data$chrom)){
    ...
    sub.ld=sqlWrapper(dbc,paste0("SELECT * FROM gene_gwas_staging.ld WHERE pos1 IN (",paste(sub.dta$pos,collapse=","),") and chrom1='",i,"'"),uniq = F, zrok = FALSE)
